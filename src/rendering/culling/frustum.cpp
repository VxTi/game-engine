...
void Frustum::updateViewProjectionMatrix(glm::mat4 viewMatrix,
...
bool Frustum::isWithin(Transform &observer, glm::vec3 referencePoint,
  ...
  for (int i = 0; i < FRUSTUM_PLANES; i++) {
    float distance =
        glm::dot(glm::vec3(rotationMatrix * glm::vec4(planes[i].normal, 1.0f)),
                 relativePosition) +
        planes[i].distance;
    if (distance < -radius) {
      return false;
...
Frustum::~Frustum() { free(this->planes); }
