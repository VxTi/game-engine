...
void Renderer::computeMatrices(float fov, float zNear, float zFar, float width, float height)
{
    if ( Renderer::renderMode == RENDER_MODE_3D ) {
        ...
        Renderer::currentMatrix.modelMatrix = glm::scale(Renderer::currentMatrix.modelMatrix,
                                                         Renderer::currentMatrix.scale);
        Renderer::currentMatrix.modelMatrix = glm::rotate(Renderer::currentMatrix.modelMatrix,
                                                          Renderer::currentMatrix.rotation.z,
                                                          glm::vec3(0.0f, 0.0f, 1.0f));
        Renderer::currentMatrix.modelMatrix = glm::rotate(Renderer::currentMatrix.modelMatrix,
                                                          Renderer::currentMatrix.rotation.x,
                                                          glm::vec3(1.0f, 0.0f, 0.0f));
        Renderer::currentMatrix.modelMatrix = glm::rotate(Renderer::currentMatrix.modelMatrix,
                                                          Renderer::currentMatrix.rotation.y,
                                                          glm::vec3(0.0f, 1.0f, 0.0f));
        Renderer::currentMatrix.modelMatrix = glm::translate(Renderer::currentMatrix.modelMatrix,
                                                             glm::vec3(-Renderer::currentMatrix.position));
        Renderer::currentMatrix.projectionMatrix = glm::perspective(glm::radians(fov),
                                                                    width / height,
                                                                    zNear, zFar);
        Renderer::currentMatrix.viewMatrix = glm::lookAt(
        ...
        Renderer::currentMatrix.modelMatrix = glm::translate(glm::mat4(1.0f),
                                                             glm::vec3(Renderer::currentMatrix.position.x,
                                                                       Renderer::currentMatrix.position.y, 0.0f));
        Renderer::currentMatrix.modelMatrix = glm::rotate(Renderer::currentMatrix.modelMatrix,
                                                          Renderer::currentMatrix.rotation.z,
                                                          glm::vec3(0.0f, 0.0f, 1.0f));
        Renderer::currentMatrix.projectionMatrix = glm::ortho(0.0f, width, height, 0.0f, zNear, zFar);
    }
}

void Renderer::updateFrustum(Frustum *frustum)
{
    frustum->updateViewProjectionMatrix(Renderer::currentMatrix.viewMatrix, Renderer::currentMatrix.projectionMatrix);
}
...
void Renderer::pushMatrices(GLuint shaderProgramId)
{
    glUniformMatrix4fv(
            ...
            1, GL_FALSE, glm::value_ptr(Renderer::currentMatrix.modelMatrix)
    );
    glUniformMatrix4fv(
            glGetUniformLocation(shaderProgramId, "u_ViewMatrix"),
            1, GL_FALSE, glm::value_ptr(Renderer::currentMatrix.viewMatrix)
    );
    glUniformMatrix4fv(
            glGetUniformLocation(shaderProgramId, "u_ProjectionMatrix"),
            1, GL_FALSE, glm::value_ptr(Renderer::currentMatrix.projectionMatrix));
    ...
    glUniformMatrix4fv(
            ...
            1, GL_FALSE, glm::value_ptr(Renderer::currentMatrix.modelViewProjectionMatrix)
    );
...
{
    Renderer::currentMatrix.position = pos;
}
...
{
    Renderer::currentMatrix.position.x = x;
}

void Renderer::translateY(float y)
{
    Renderer::currentMatrix.position.y = y;
}

void Renderer::translateZ(float z)
{
    Renderer::currentMatrix.position.z = z;
}
...
{
    Renderer::currentMatrix.scale.x = x;
}

void Renderer::scaleY(float y)
{
    Renderer::currentMatrix.scale.y = y;
}

void Renderer::scaleZ(float z)
{
    Renderer::currentMatrix.scale.z = z;
}
